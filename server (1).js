...
function makeBall() {
  return {
    ...
    spin: 0, inFlight: false, ownerIdx: 0,
...
function tickPlayers(g) {
  for (const p of g.players) {
    ...
    // AI assist for players with no human input this tick
    if (!p._hasInput) {
...
function releaseShot(g, p) {
  ...
  const acc = (hasStar || perfect) ? 0.97 : inGreen ? 0.85 : is3 ? 0.45 : 0.60;
  const makes = Math.random() < acc;
  if (hasStar && p.star > 0) p.star--;
  p.hasBall = false; p.shotCD = 0.9; p.jumpV = 0.18;
  const b = g.ball;
  b.inFlight = true; b.shotByIdx = p.id; b.passToIdx = -1; b.ownerIdx = -1;
  b.willScore = makes; b.is3 = is3;
  b.meterQuality = perfect ? 'perfect' : inGreen ? 'good' : 'bad';
  b.sWX = p.wx; b.sWZ = p.wz;
  b.eWX = makes ? hoop.wx : hoop.wx + (Math.random() - 0.5) * 3;
  b.eWZ = makes ? hoop.wz : hoop.wz + (Math.random() - 0.5) * 3;
...
function doSteal(g, p) {
  ...
  if (g.ball.inFlight && g.ball.passToIdx < 0) {
    ...
    if (dist2(b.wx, b.wz, p.wx, p.wz) < 2 && b.wy < 2.5) {
      b.inFlight = false; p.hasBall = true; b.ownerIdx = p.id;
      b.wx = p.wx; b.wz = p.wz; b.wy = 0.3; p.stealCD = 0.5;
      g.toast = 'BLOCKED! ✋'; g.toastColor = '#ffffff';
    }
  }
}
...
function tickBall(g) {
  ...
  if (b.inFlight) {
    ...
    if (b.passToIdx >= 0 && t >= 0.95) {
      ...
      b.inFlight = false; recv.hasBall = true; b.ownerIdx = recv.id; b.passToIdx = -1; b.wy = 0.3;
      return;
    }
    if (t >= 1.0) {
      if (b.willScore) {
        ...
        g.flashT = 0.5; shooter.celebrating = 1.4; shooter.jumpV = 0.25;
      } else {
      ...
      b.wx = recv.wx; b.wz = recv.wz; b.wy = 0.3; b.shotByIdx = -1;
    }
  } else {
    const owner = b.ownerIdx >= 0 ? g.players[b.ownerIdx] : null;
    if (owner) { b.wx = lerp(b.wx, owner.wx, 0.35); b.wz = lerp(b.wz, owner.wz, 0.35); b.wy = 0.3; }
  }
}
...
function doBomb(g, p) {
  ...
  for (const o of g.players) {
    ...
    if (dist2(p.wx, p.wz, o.wx, o.wz) < 3.5) {
      ...
      o.stunned = 1.5;
      if (o.hasBall) { o.hasBall = false; p.hasBall = true; g.ball.ownerIdx = p.id; g.ball.wx = p.wx; g.ball.wz = p.wz; }
      const dx = o.wx - p.wx || 0.1, dz = o.wz - p.wz || 0.1, d = Math.hypot(dx, dz) || 1;
      o.wx = clamp(o.wx + dx/d * 3, 0.5, CW - 0.5);
      o.wz = clamp(o.wz + dz/d * 3, 0.5, CD - 0.5);
    }
  }
}
...
function tickProjectiles(g) {
  g.projectiles = g.projectiles.filter(pr => {
    ...
    if (pr.type === 'rocket' && pr.targetIdx >= 0) {
      ...
      if (tgt) {
        const dx = tgt.wx - pr.wx, dz = tgt.wz - pr.wz, d = Math.max(0.1, Math.hypot(dx, dz));
        pr.vx = lerp(pr.vx, dx/d * 0.25, 0.08);
        pr.vz = lerp(pr.vz, dz/d * 0.25, 0.08);
      }
    }
    for (const p of g.players) {
      if (p.id === pr.ownerIdx) continue;
      const hr = pr.type === 'rocket' ? 2 : 1.5;
      if (dist2(pr.wx, pr.wz, p.wx, p.wz) < hr) {
        if (p.shield) { p.shield = false; p.shieldT = 0; return false; }
        if (pr.type === 'rocket') {
          p.stunned = 2; g.flashT = 0.4;
          g.toast = '🚀 DIRECT HIT!'; g.toastColor = '#ff6600';
          if (p.hasBall) { p.hasBall = false; g.ball.inFlight = false; g.ball.ownerIdx = -1; g.ball.wx = p.wx; g.ball.wz = p.wz; g.ball.wy = 0.3; }
        } else {
          p.stunned = 0.8;
        }
        return false;
...
function tickItemBoxes(g) {
  for (const box of g.itemBoxes) {
    if (!box.alive) {
      box.respawn -= DT;
      if (box.respawn <= 0) { box.alive = true; box.item = randItem(); }
      continue;
    }
    for (const p of g.players) {
      if (p.item) continue;
      if (dist2(p.wx, p.wz, box.wx, box.wz) < 1.6) {
        p.item = box.item; box.alive = false; box.respawn = 12;
        g.toast = `${ITEM_ICONS[box.item]} ${ITEM_NAMES[box.item]}`;
        g.toastColor = ITEM_COLORS[box.item];
      }
    }
  }
}
...
function tickTornado(g) {
  if (!g.tornado) return;
  g.tornado.t -= DT;
  if (g.tornado.t <= 0) { g.tornado = null; return; }
  const ang = g.tornado.t * 2;
  g.tornado.wx = MX + Math.cos(ang) * 6;
  g.tornado.wz = MZ + Math.sin(ang) * 3.5;
  for (const p of g.players) {
    if (p.id === g.tornado.ownerIdx) continue;
    if (dist2(p.wx, p.wz, g.tornado.wx, g.tornado.wz) < 3.5) {
      const dx = p.wx - g.tornado.wx || 0.1, dz = p.wz - g.tornado.wz || 0.1;
      const d = Math.hypot(dx, dz) || 1;
      p.wx = clamp(p.wx + dx/d * 0.15, 0.5, CW - 0.5);
      p.wz = clamp(p.wz + dz/d * 0.15, 0.5, CD - 0.5);
    }
  }
}
...
function endQuarter(room) {
  ...
  g.players.forEach(p => {
    p.hasBall = false; p.celebrating = 0; p.jumpH = 0; p.jumpV = 0;
    p.shooting = false; p.frozen = 0; p.stunned = 0;
    p.mega = 0; p.speed = 0; p.shield = false; p.star = 0; p.gunAmmo = 0; p.item = null;
  });
  [[MX-4,MZ-1.5],[MX-4,MZ+1.5],[MX+4,MZ-1.5],[MX+4,MZ+1.5]].forEach(([wx,wz], i) => {
    g.players[i].wx = wx; g.players[i].wz = wz;
  });
...
io.on('connection', socket => {
  ...
  socket.on('joinRoom', ({ code, name }) => {
    ...
    socket.emit('joinedRoom', { code: roomCode, team, playerIdx });
    io.to(roomCode).emit('playerList', room.players.map(p => ({ name: p.name, team: p.team, playerIdx: p.playerIdx })));
    console.log(`${pname} joined room ${roomCode} as P${playerIdx + 1}`);
  ...
  socket.on('input', input => {
    ...
    const g = room.game;
    const p = g.players[socket.playerIdx];
    ...
    if (mx !== 0 || mz !== 0) {
      ...
      p.wx += mx / len * spd; p.wz += mz / len * spd;
      p.facing = mx > 0 ? 1 : -1;
      p._hasInput = true;
      p.wx = clamp(p.wx, 0.5, CW - 0.5);
      p.wz = clamp(p.wz, 0.5, CD - 0.5);
  ...
  socket.on('disconnect', () => {
    ...
    if (room.players.length === 0) {
      ...
      console.log(`Room ${socket.roomCode} destroyed`);
    } else {
      io.to(socket.roomCode).emit('playerLeft', { playerIdx: socket.playerIdx, playersLeft: room.players.length });
    }
  });
});
  ...
  room._interval = setInterval(() => {
    ...
    io.to(room.code).emit('state', {
      players: g.players.map(p => ({
        ...
        shield: p.shield, star: p.star, gunAmmo: p.gunAmmo,
        item: p.item, bobT: +p.bobT.toFixed(3),
      })),
      ball: {
        ...
        meterQuality: g.ball.meterQuality,
      },
      ...
      toast: g.toast, toastColor: g.toastColor,
    });
  }, TICK_MS);
}

const PORT = process.env.PORT || 3000;
server.listen(PORT, () => console.log(`🏀 Hardwood Chaos server running on port ${PORT}`));
